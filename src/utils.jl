...
# `Core.Typeof`, not a hand-built `isa(x, Type) ? Type{x} : typeof(x)`: the
# two agree through Julia 1.13, but on 1.14 (TypeEgal, JuliaLang/julia#61915)
# type-valued arguments dispatch through `Core.TypeEgal` and the hand-built
# `Type{x}` signature resolves calls like `getproperty(::Type, :name)` to the
# wrong method (observed: `FieldError: Core.TypeEgal has no field name`, plus
# mis-dispatch-induced stack overflows). `Core.Typeof` is the
# dispatch-accurate spelling on every version; the full suite passes with it
# on 1.12.6 and the TypeEgal trigger agrees with native on 1.14-DEV.
